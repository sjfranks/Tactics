...
function blockedTerrain(c:CanvasRenderingContext2D,svg:SVGSVGElement){for(const el of svg.querySelectorAll<SVGRectElement>('.blocked-range')){const x=Math.round((n(el,'x')-2)/CELL)*CELL,y=Math.round((n(el,'y')-2)/CELL)*CELL;c.save();c.fillStyle='rgba(91,99,109,.34)';c.fillRect(x+.5,y+.5,CELL-1,CELL-1);c.beginPath();c.rect(x+.5,y+.5,CELL-1,CELL-1);c.clip();c.strokeStyle='rgba(226,231,236,.64)';c.lineWidth=1.25;for(let offset=-CELL;offset<CELL*2;offset+=5){c.beginPath();c.moveTo(x+offset,y+CELL);c.lineTo(x+offset+CELL,y);c.stroke()}c.restore()}}
