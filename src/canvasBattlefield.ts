...
.game-board[aria-label="6 by 8 tactical battlefield"] .tiles>*,.game-board[aria-label="6 by 8 tactical battlefield"] .highlights>*,.game-board[aria-label="6 by 8 tactical battlefield"] .props>*,.game-board[aria-label="6 by 8 tactical battlefield"] .tokens>*{opacity:0!important}
...
const portraitCells:Record<string,{col:number;row:number}>={
 alden:{col:0,row:0},mira:{col:1,row:0},'raider-1':{col:1,row:2},'raider-2':{col:1,row:2}
};
...
function box(c:CanvasRenderingContext2D,x:number,y:number,fill:string,stroke:string,line=2){c.fillStyle=fill;c.strokeStyle=stroke;c.lineWidth=line;rr(c,x+3,y+3,CELL-6,CELL-6,7);c.fill();c.stroke()}
...
function playerThreat(c:CanvasRenderingContext2D,svg:SVGSVGElement){
 ...
 for(const el of svg.querySelectorAll<SVGRectElement>('.move-range')){
  ...
  for(const [dx,dy] of [[0,0],[1,0],[-1,0],[0,1],[0,-1]]){const nx=x+dx,ny=y+dy;if(nx>=0&&nx<6&&ny>=0&&ny<8)cells.add(`${nx},${ny}`)}
 }
...
function grid(c:CanvasRenderingContext2D){c.save();c.strokeStyle='rgba(12,17,10,.62)';c.lineWidth=1.35;for(let x=0;x<=480;x+=CELL){c.beginPath();c.moveTo(x,0);c.lineTo(x,640);c.stroke()}for(let y=0;y<=640;y+=CELL){c.beginPath();c.moveTo(0,y);c.lineTo(480,y);c.stroke()}c.restore()}
...
function drawPortrait(c:CanvasRenderingContext2D,id:string){
 const cell=portraitCells[id];if(!cell||!portraits.complete||!portraits.naturalWidth)return;
 const cw=portraits.naturalWidth/5,ch=portraits.naturalHeight/3,side=Math.min(cw,ch)-12;
 const sx=cell.col*cw+(cw-side)/2,sy=cell.row*ch+(ch-side)/2;
...
function token(c:CanvasRenderingContext2D,g:SVGGElement){
 if(g.classList.contains('drag-ghost'))return;const id=g.dataset.id??'';if(!portraitCells[id])return;const p=tr(g),enemy=g.classList.contains('enemy'),active=g.classList.contains('active');
...
function draw(svg:SVGSVGElement){const r=frame.getBoundingClientRect();if(r.width<2||r.height<2)return;const dpr=Math.min(devicePixelRatio||1,3),w=Math.round(r.width*dpr),h=Math.round(r.height*dpr);if(w!==backingW||h!==backingH){backingW=w;backingH=h;canvas.width=w;canvas.height=h}const s=Math.min(r.width/480,r.height/640),ox=(r.width-480*s)/2,cam=camera(svg);mapArt.style.left=`${ox}px`;mapArt.style.top='0px';mapArt.style.width=`${480*s}px`;mapArt.style.height=`${640*s}px`;mapArt.style.transform=`translate(${cam.x*s}px,${cam.y*s}px) scale(${cam.z})`;const c=canvas.getContext('2d')!;c.setTransform(dpr,0,0,dpr,0,0);c.clearRect(0,0,r.width,r.height);c.save();c.translate(ox+cam.x*s,cam.y*s);c.scale(s*cam.z,s*cam.z);highlights(c,svg);grid(c);corners(c,svg);for(const g of svg.querySelectorAll<SVGGElement>('.unit-token[data-id]'))token(c,g);c.restore()}
let lastTactical:SVGSVGElement|undefined;
function loop(){const svg=latest();if(svg&&svg.viewBox.baseVal.width===480&&svg.viewBox.baseVal.height===640){lastTactical=svg;mapArt.style.display='block';canvas.style.display='block';draw(svg)}else if(!svg&&lastTactical){mapArt.style.display='block';canvas.style.display='block'}else if(svg){lastTactical=undefined;mapArt.style.display='none';canvas.style.display='none'}requestAnimationFrame(loop)}
