...
.battlefield-map-art{position:absolute;z-index:1;pointer-events:none;display:block;object-fit:fill;transform-origin:0 0;will-change:transform;image-rendering:auto}
...
function drawPortrait(c:CanvasRenderingContext2D,id:string){
 ...
 // Owlbear-style image item: preserve the asset's square geometry and center it in the token.
 const side=Math.min(im.naturalWidth,im.naturalHeight),sx=(im.naturalWidth-side)/2,sy=(im.naturalHeight-side)/2;
 c.drawImage(im,sx,sy,side,side,-23,-23,46,46);
}
