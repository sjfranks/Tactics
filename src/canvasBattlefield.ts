...
const terrain=new Image();terrain.decoding='async';terrain.src=`${import.meta.env.BASE_URL}assets/emblem/terrain.png`;
...
function tile(c:CanvasRenderingContext2D,index:number,x:number,y:number){if(!terrain.complete||!terrain.naturalWidth)return;const sw=terrain.naturalWidth/ATLAS_COLS,sh=terrain.naturalHeight/ATLAS_COLS,pad=2,sx=index%ATLAS_COLS*sw+pad,sy=Math.floor(index/ATLAS_COLS)*sh+pad;c.drawImage(terrain,sx,sy,sw-pad*2,sh-pad*2,x,y,CELL,CELL)}
...
function token(c:CanvasRenderingContext2D,g:SVGGElement){if(g.classList.contains('drag-ghost'))return;const id=g.dataset.id??'',cell=unitCells[id];if(!cell||!units.complete||!units.naturalWidth)return;const p=tr(g),enemy=g.classList.contains('enemy'),active=g.classList.contains('active');c.save();c.translate(Math.round(p.x),Math.round(p.y));if(active){c.fillStyle=enemy?'rgba(240,70,80,.48)':'rgba(67,190,255,.52)';c.beginPath();c.ellipse(0,6,7.5,2.6,0,0,Math.PI*2);c.fill()}const sw=units.naturalWidth/3,sh=units.naturalHeight/2;c.drawImage(units,cell[0]*sw,cell[1]*sh,sw,sh,-7.5,-8,15,15);const hp=g.querySelector<SVGRectElement>('.hp-fill'),ratio=hp?Math.min(1,n(hp,'width',12)/12):1;c.fillStyle='#172239';c.fillRect(-7,6,14,3);c.fillStyle='#f2e6c5';c.fillRect(-6.5,6.5,13,2);c.fillStyle=enemy?'#e94b58':'#58c96a';c.fillRect(-6.5,6.5,Math.round(13*ratio),2);if(g.querySelector('.shield-mark')){c.save();c.translate(5,-6);c.fillStyle='#f5e6b7';c.strokeStyle='#234f82';c.lineWidth=.8;c.beginPath();c.moveTo(0,-3);c.lineTo(3,-2);c.lineTo(2.5,1);c.quadraticCurveTo(1.5,3,0,4);c.quadraticCurveTo(-1.5,3,-2.5,1);c.lineTo(-3,-2);c.closePath();c.fill();c.stroke();c.restore()}c.restore()}
