...
function attackCues(c:CanvasRenderingContext2D,svg:SVGSVGElement){for(const g of svg.querySelectorAll<SVGGElement>('.attack-action-cue')){const p=tr(g),ranged=g.classList.contains('ranged');c.save();c.translate(p.x,p.y);c.shadowColor='rgba(21,8,11,.42)';c.shadowBlur=1;c.shadowOffsetY=1;c.fillStyle='#f3a1aa';c.strokeStyle='#7f1d2d';c.lineWidth=1.15;c.beginPath();c.arc(0,0,4.7,0,Math.PI*2);c.fill();c.stroke();c.shadowColor='transparent';c.shadowBlur=0;c.shadowOffsetY=0;c.fillStyle='#bd2940';c.scale(.74,.74);if(ranged){c.rotate(42*Math.PI/180);c.beginPath();c.moveTo(-.62,4.1);c.lineTo(.62,4.1);c.lineTo(.62,-1.15);c.lineTo(2.35,-1.15);c.lineTo(0,-4.25);c.lineTo(-2.35,-1.15);c.lineTo(-.62,-1.15);c.closePath();c.fill()}else{sword(c,-43);sword(c,43)}c.restore()}}
