...
function tile(c:CanvasRenderingContext2D,index:number,x:number,y:number){if(!terrain.complete||!terrain.naturalWidth)return;const sw=terrain.naturalWidth/ATLAS_COLS,sh=terrain.naturalHeight/ATLAS_COLS,sx=index%ATLAS_COLS*sw,sy=Math.floor(index/ATLAS_COLS)*sh;c.drawImage(terrain,sx,sy,sw,sh,x,y,CELL,CELL)}
...
function grid(_c:CanvasRenderingContext2D,_w:number,_h:number){}
function selectedId(){return document.querySelector<HTMLElement>('#portrait')?.dataset.id??''}
function selection(c:CanvasRenderingContext2D,svg:SVGSVGElement){const g=svg.querySelector<SVGGElement>('.unit-token.active');if(!g)return;const p=tr(g),enemy=g.classList.contains('enemy'),x=Math.round(p.x)-8,y=Math.round(p.y)-8;c.save();c.fillStyle=enemy?'rgba(205,48,64,.25)':'rgba(39,151,230,.25)';c.fillRect(x+.75,y+.75,14.5,14.5);c.strokeStyle=enemy?'#751e35':'#163e77';c.lineWidth=2;c.strokeRect(x+1,y+1,14,14);c.strokeStyle=enemy?'#ff9a9f':'#baf2ff';c.lineWidth=.9;c.strokeRect(x+2,y+2,12,12);c.strokeStyle=enemy?'#fff0dc':'#ffffff';c.lineWidth=1.4;const k=4,edges=[[x+1,y+1,x+1+k,y+1],[x+1,y+1,x+1,y+1+k],[x+15,y+1,x+15-k,y+1],[x+15,y+1,x+15,y+1+k],[x+1,y+15,x+1+k,y+15],[x+1,y+15,x+1,y+15-k],[x+15,y+15,x+15-k,y+15],[x+15,y+15,x+15,y+15-k]];for(const e of edges){c.beginPath();c.moveTo(e[0],e[1]);c.lineTo(e[2],e[3]);c.stroke()}c.restore()}
