...
function token(c:CanvasRenderingContext2D,g:SVGGElement){const ghost=g.classList.contains('drag-ghost'),id=g.dataset.id??'',cell=unitCells[id];if(!cell||!units.complete||!units.naturalWidth)return;const p=tr(g),enemy=g.classList.contains('enemy');c.save();c.translate(Math.round(p.x),Math.round(p.y));if(ghost){c.globalAlpha=.62;c.shadowColor='rgba(91,224,255,.9)';c.shadowBlur=2.5}const sw=units.naturalWidth/3,sh=units.naturalHeight/2;c.drawImage(units,cell[0]*sw,cell[1]*sh,sw,sh,-7.5,-8,15,15);if(ghost){c.restore();return}const hp=g.querySelector<SVGRectElement>('.hp-fill'),ratio=hp?Math.min(1,n(hp,'width',12)/12):1;c.fillStyle='#172239';c.fillRect(-7,6,14,3);c.fillStyle='#f2e6c5';c.fillRect(-6.5,6.5,13,2);c.fillStyle=enemy?'#e94b58':'#58c96a';c.fillRect(-6.5,6.5,Math.round(13*ratio),2);if(g.querySelector('.shield-mark')){c.save();c.translate(5,-6);c.fillStyle='#f5e6b7';c.strokeStyle='#234f82';c.lineWidth=.8;c.beginPath();c.moveTo(0,-3);c.lineTo(3,-2);c.lineTo(2.5,1);c.quadraticCurveTo(1.5,3,0,4);c.quadraticCurveTo(-1.5,3,-2.5,1);c.lineTo(-3,-2);c.closePath();c.fill();c.stroke();c.restore()}c.restore()}

let backingW=0,backingH=0;
function draw(svg:SVGSVGElement){const r=frame.getBoundingClientRect();if(r.width<2||r.height<2)return;const dpr=Math.min(devicePixelRatio||1,3),w=Math.round(r.width*dpr),h=Math.round(r.height*dpr);if(w!==backingW||h!==backingH){backingW=w;backingH=h;canvas.width=w;canvas.height=h;tokenCanvas.width=w;tokenCanvas.height=h}const worldW=svg.viewBox.baseVal.width,worldH=svg.viewBox.baseVal.height,s=Math.min(r.width/worldW,r.height/worldH),ox=(r.width-worldW*s)/2,cam=camera(svg),c=canvas.getContext('2d')!,tc=tokenCanvas.getContext('2d')!;for(const context of[c,tc]){context.imageSmoothingEnabled=true;context.imageSmoothingQuality='high';context.setTransform(dpr,0,0,dpr,0,0);context.clearRect(0,0,r.width,r.height);context.save();context.translate(ox+cam.x*s,cam.y*s);context.scale(s*cam.z,s*cam.z)}backdrop(c,worldW,worldH);highlights(c,svg);blockedTerrain(c,svg);grid(c,worldW,worldH);selection(tc,svg);for(const g of svg.querySelectorAll<SVGGElement>('.unit-token[data-id]'))token(tc,g);c.restore();tc.restore()}
function loop(){const svg=latest();if(svg?.getAttribute('aria-label')?.endsWith('tactical battlefield')){canvas.style.display='block';tokenCanvas.style.display='block';draw(svg)}else{canvas.style.display='none';tokenCanvas.style.display='none'}requestAnimationFrame(loop)}
