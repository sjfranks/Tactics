...
class Game{
  ...
  drawAttackCue(t:Unit,kind:'melee'|'ranged'){if(!this.routeLayer)return;const q={x:t.x*CELL+CELL/2+5,y:t.y*CELL+CELL/2-8},cue=svg('g',{transform:`translate(${q.x} ${q.y})`,class:`attack-action-cue ${kind}`}),icon=kind==='ranged'?'M -3.6 2.9 L 3.3 -2.9 M .5 -3.4 L 3.3 -2.9 L 2.8 -.1 M -3.7 .7 L -1.6 2.8 M -2.6 -.2 L -.5 1.9':'M -3.2 -3.1 L 2.5 2.6 M .7 2.9 L 3 3.1 L 2.8 .8 M 3.2 -3.1 L -2.5 2.6 M -.7 2.9 L -3 3.1 L -2.8 .8';cue.append(svg('circle',{cx:0,cy:0,r:6}),svg('path',{d:icon}));this.routeLayer.appendChild(cue);}
  showEnemyAttack(t:Unit,kind:'melee'|'ranged',act:()=>void){this.render();this.drawAttackCue(t,kind);this.schedule(460,act);}
  async runEnemy(){const e=this.active();if(!e||e.team!=='enemy'||this.gameOver||this.busy)return;this.selectedId=e.id;const kind=this.weaponKind(e);let target:Unit|undefined,bestRoute:Point[]=[];for(const hero of this.living('player')){const r=this.routeToRange(e,hero,kind);if(r.length&&(!bestRoute.length||r.length<bestRoute.length)){target=hero;bestRoute=r;}}if(!target){target=[...this.living('player')].sort((a,b)=>this.distance(e,a)-this.distance(e,b))[0];}if(!target){this.finishTurn();return;}if(this.canAttack(e,target,kind)){this.showEnemyAttack(target,kind,()=>void this.attack(e,target!,kind));return;}if(bestRoute.length>1){const step=bestRoute.slice(0,Math.min(bestRoute.length,MOVE+1));await this.moveEnemy(e,step);}const liveTarget=this.unit(target.id);if(liveTarget&&this.canAttack(e,liveTarget,kind))this.showEnemyAttack(liveTarget,kind,()=>void this.attack(e,liveTarget,kind));else this.schedule(260,()=>this.defendUnit(e));}
  ...
  syncUI(){const a=this.active(),playerTurn=!!a&&a.team==='player'&&!this.gameOver,kind=a?this.weaponKind(a):'melee';ui.undo.disabled=!a||a.team!=='player'||!this.history.length;ui.actionToggle.hidden=!playerTurn;ui.actionToggle.style.visibility='';ui.actionToggle.setAttribute('aria-expanded',String(this.actionsOpen));ui.actionBar.hidden=!playerTurn||!this.actionsOpen||this.gameOver;ui.actionBar.dataset.team=a?.team??'';ui.zoomReset.disabled=this.cameraMode==='clean'&&this.scale===1&&this.tx===0&&this.ty===0;const hasTarget=!!a&&this.living(a.team==='player'?'enemy':'player').some(t=>this.canAttack(a,t,kind)),canAct=!!a&&this.hasAction(a);ui.attackAction.hidden=false;ui.attackAction.disabled=!canAct||!hasTarget;ui.attackAction.setAttribute('aria-pressed',String(this.targetingMode==='attack'));ui.heal.hidden=a?.id!=='mira';ui.heal.disabled=!playerTurn||!this.hasAction(a!)||!this.living('player').some(u=>this.distance(a!,u)<=HEAL_RANGE&&u.hp<u.maxHp);ui.defend.disabled=!canAct||!!a?.defending;ui.defend.setAttribute('aria-pressed','false');ui.heal.setAttribute('aria-pressed',String(this.targetingMode==='heal'));ui.tactical.setAttribute('aria-pressed',String(this.mode==='tactical'));ui.exploration.setAttribute('aria-pressed',String(this.mode==='exploration'));
    const selected=this.selected()??a??this.living()[0];if(selected){ui.drawer.dataset.team=selected.team;ui.selectedName.textContent=selected.name;ui.selectedTeam.textContent=selected.team==='player'?'Hero':'Enemy';ui.health.textContent=`${selected.hp} / ${selected.maxHp}`;ui.attack.textContent=String(selected.damage);ui.movement.textContent=String(MOVE);ui.initiative.textContent=`${selected.initiativeMod>=0?'+':''}${selected.initiativeMod} (${selected.initiativeScore})`;ui.portrait.textContent='';ui.portrait.dataset.id=selected.id;}
    ui.rail.innerHTML='';for(const [i,id] of this.turnOrder.entries()){const u=this.unit(id);if(!u)continue;const b=document.createElement('button');b.type='button';b.className=`initiative-token ${u.team}${i===this.activeIndex?' active':''}${u.id===this.selectedId?' selected':''}`;b.dataset.id=u.id;b.setAttribute('aria-label',`${u.name}${i===this.activeIndex?', current turn':''}`);b.innerHTML=`<span class="rail-sprite"></span><small>${u.name}</small>`;b.addEventListener('click',()=>{this.targetingMode=undefined;this.pendingAttack=undefined;this.hideForecast();this.selectedId=u.id;this.scale=1;this.tx=0;this.ty=0;this.cameraMode='clean';this.render();});ui.rail.appendChild(b);}const current=ui.rail.querySelector<HTMLElement>('.active');if(current)requestAnimationFrame(()=>current.scrollIntoView({behavior:'smooth',inline:'center',block:'nearest'}));}
