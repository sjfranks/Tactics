...
class Game{
  ...
  showEnemyChoice(choice:'attack'|'defend',act:()=>void){this.actionsOpen=true;this.render();const button=choice==='attack'?ui.attackAction:ui.defend;button.disabled=false;requestAnimationFrame(()=>{button.classList.add('enemy-choice');button.setAttribute('aria-pressed','true');});this.schedule(620,act);}
  async runEnemy(){const e=this.active();if(!e||e.team!=='enemy'||this.gameOver||this.busy)return;this.selectedId=e.id;const kind=this.weaponKind(e);let target:Unit|undefined,bestRoute:Point[]=[];for(const hero of this.living('player')){const r=this.routeToRange(e,hero,kind);if(r.length&&(!bestRoute.length||r.length<bestRoute.length)){target=hero;bestRoute=r;}}if(!target){target=[...this.living('player')].sort((a,b)=>this.distance(e,a)-this.distance(e,b))[0];}if(!target){this.finishTurn();return;}if(this.canAttack(e,target,kind)){this.showEnemyChoice('attack',()=>void this.attack(e,target!,kind));return;}if(bestRoute.length>1){const step=bestRoute.slice(0,Math.min(bestRoute.length,MOVE+1));await this.moveEnemy(e,step);}const liveTarget=this.unit(target.id);if(liveTarget&&this.canAttack(e,liveTarget,kind))this.showEnemyChoice('attack',()=>void this.attack(e,liveTarget,kind));else this.showEnemyChoice('defend',()=>this.defendUnit(e));}
