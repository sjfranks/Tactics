...
class Game{
  ...
  paintHighlights(layer:SVGGElement){
    ...
    if(this.targetingMode&&a?.team==='player'){
      ...
      const kind=this.weaponKind(a),range=this.weaponRange(a);for(let y=0;y<this.rows;y++)for(let x=0;x<this.cols;x++){const d=this.distance(a,{x,y});if(d>0&&d<=range)add({x,y},'weapon-range');}this.living('enemy').filter(u=>this.canAttack(a,u,kind)).forEach(u=>add(u,'attack-range'));return;
    }
  ...
  handleUnit(id:string){if(this.busy||this.gameOver||this.gameMode!=='combat')return;const t=this.unit(id);if(!t)return;const a=this.active();if(this.targetingMode&&a?.team==='player'){if(this.targetingMode==='heal'){if(t.team==='player'&&this.distance(a,t)<=HEAL_RANGE&&t.hp<t.maxHp)this.healTarget(t);else this.message(`Choose an injured hero within ${HEAL_RANGE} square.`);return;}const kind=this.weaponKind(a);if(t.team==='enemy'&&this.canAttack(a,t,kind)){void this.attack(a,t,kind);return;}this.message(kind==='ranged'?`Choose an enemy exactly ${RANGED_RANGE} squares away.`:'Choose an adjacent enemy.');return;}this.pendingAttack=undefined;this.hideForecast();this.selectedId=id;this.render();}

  beginTargeting(kind:TargetingMode){const a=this.active();if(!a||a.team!=='player'||!this.hasAction(a)||this.busy)return;if(kind==='heal'&&a.id!=='mira')return;if(this.targetingMode===kind){this.targetingMode=undefined;this.pendingAttack=undefined;this.hideForecast();this.render();this.message('Action cancelled.');return;}this.targetingMode=kind;this.actionsOpen=false;this.pendingAttack=undefined;this.hideForecast();this.selectedId=a.id;this.render();this.message(kind==='heal'?`Choose an injured hero within ${HEAL_RANGE} square.`:this.weaponKind(a)==='ranged'?`Choose an enemy exactly ${RANGED_RANGE} squares away.`:'Choose an adjacent enemy.');}
  canAttack(a:Unit,t:Unit,kind:'melee'|'ranged'=this.weaponKind(a)){const d=this.distance(a,t);return this.hasAction(a)&&a.team!==t.team&&(kind==='ranged'?d===RANGED_RANGE:d===1);}
