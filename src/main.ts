...
class Game{
  ...
  showEnemyAttack(e:Unit,act:()=>void){this.render();const q={x:e.x*CELL+CELL/2,y:e.y*CELL+CELL/2-9},cue=svg('g',{transform:`translate(${q.x} ${q.y})`,class:'enemy-attack-cue'});cue.append(svg('circle',{cx:0,cy:0,r:5}),svg('path',{d:'M -2.8 2.8 L 2.8 -2.8 M .8 -3 L 3 -3 L 3 -.8 M -3 .4 L -.4 3'}));this.routeLayer?.appendChild(cue);this.schedule(460,act);}
  async runEnemy(){const e=this.active();if(!e||e.team!=='enemy'||this.gameOver||this.busy)return;this.selectedId=e.id;const kind=this.weaponKind(e);let target:Unit|undefined,bestRoute:Point[]=[];for(const hero of this.living('player')){const r=this.routeToRange(e,hero,kind);if(r.length&&(!bestRoute.length||r.length<bestRoute.length)){target=hero;bestRoute=r;}}if(!target){target=[...this.living('player')].sort((a,b)=>this.distance(e,a)-this.distance(e,b))[0];}if(!target){this.finishTurn();return;}if(this.canAttack(e,target,kind)){this.showEnemyAttack(e,()=>void this.attack(e,target!,kind));return;}if(bestRoute.length>1){const step=bestRoute.slice(0,Math.min(bestRoute.length,MOVE+1));await this.moveEnemy(e,step);}const liveTarget=this.unit(target.id);if(liveTarget&&this.canAttack(e,liveTarget,kind))this.showEnemyAttack(e,()=>void this.attack(e,liveTarget,kind));else this.schedule(260,()=>this.defendUnit(e));}
