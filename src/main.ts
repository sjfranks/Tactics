...
class Game{
  ...
  drawAttackCue(t:Unit,kind:'melee'|'ranged'){if(!this.routeLayer)return;const q={x:t.x*CELL+CELL/2+6,y:t.y*CELL+CELL/2-8},cue=svg('g',{transform:`translate(${q.x} ${q.y})`,class:'attack-action-cue'}),icon=kind==='ranged'?'M -3 2.5 L 3 -2.5 M .5 -3 L 3 -2.5 L 2.4 0':'M -2.8 2.8 L 2.8 -2.8 M .8 -3 L 3 -3 L 3 -.8 M -3 .4 L -.4 3';cue.append(svg('circle',{cx:0,cy:0,r:5}),svg('path',{d:icon}));this.routeLayer.appendChild(cue);}
