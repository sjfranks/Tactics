...
.initiative-token img{width:100%;height:100%;display:block;object-fit:cover;object-position:50% 50%;transform:none;pointer-events:none}
.initiative-token.active img{filter:drop-shadow(0 0 5px rgba(50,188,255,.7))}
#portrait{background-position:50% 50%;background-size:cover;background-repeat:no-repeat;color:transparent;overflow:hidden}
`;
...
function replaceNames(){
  ...
  for(const b of document.querySelectorAll<HTMLButtonElement>('.initiative-token')){
    ...
    const span=b.querySelector('span');if(src&&span){span.textContent='';const img=document.createElement('img');img.src=src;img.alt='';img.dataset.artId=id;span.appendChild(img);}
  }
