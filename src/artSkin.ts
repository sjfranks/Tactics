...
function drawSelectionCorners(){
  ...
  const m=/translate\(([-.\d]+)[ ,]([-.\d]+)\)/.exec(token.getAttribute('transform')??'');if(!m)return;const cx=Number(m[1]),cy=Number(m[2]),x=cx-40,y=cy-40,p=5,l=15;
  const g=document.createElementNS(NS,'g');g.setAttribute('class',`selection-corners ${token.classList.contains('enemy')?'enemy':''}`);const path=document.createElementNS(NS,'path');
  path.setAttribute('d',`M ${x+p+l} ${y+p} H ${x+p} V ${y+p+l} M ${x+80-p-l} ${y+p} H ${x+80-p} V ${y+p+l} M ${x+p} ${y+80-p-l} V ${y+80-p} H ${x+p+l} M ${x+80-p-l} ${y+80-p} H ${x+80-p} V ${y+80-p-l}`);g.appendChild(path);layer.appendChild(g);
}
function apply(){skinBoard();replaceNames();drawSelectionCorners();}
new MutationObserver(()=>requestAnimationFrame(apply)).observe(document.body,{childList:true,subtree:true});apply();
