...
.initiative-token img{position:absolute;inset:0;width:100%;height:100%;display:block;object-fit:cover;object-position:50% 50%;transform:none!important;pointer-events:none;margin:0}
...
function replaceNames(){
 ...
 for(const b of document.querySelectorAll<HTMLButtonElement>('.initiative-token')){
  ...
  const src=id==='raider-1'?ART.goblin1:id==='raider-2'?ART.goblin2:id==='alden'?ART.alden:id==='mira'?ART.mira:'';const span=b.querySelector('span');let img=b.querySelector<HTMLImageElement>('img');
  if(src&&span&&!img){span.textContent='';img=document.createElement('img');img.src=src;img.alt='';span.appendChild(img)}if(img&&id)img.dataset.artId=id;
 }
 const portrait=document.querySelector<HTMLElement>('#portrait'),name=document.querySelector<HTMLElement>('#selected-name')?.textContent??'';const src=name.includes('Goblin Raider')?ART.goblin1:name.includes('Goblin Skirmisher')?ART.goblin2:name.includes('Alden')?ART.alden:name.includes('Mira')?ART.mira:'';if(portrait)portrait.style.backgroundImage=src?`url("${src}")`:'';
}
