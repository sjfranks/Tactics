export const dragPolish={
  ...
  transform(code:string,id:string){
    ...
    const start={x:first.x+(second.x-first.x)/sl*5,y:first.y+(second.y-first.y)/sl*5};
    const dx=last.x-prev.x,dy=last.y-prev.y,len=Math.hypot(dx,dy)||1,ux=dx/len,uy=dy/len,px=-uy,py=ux;
    const shaftEnd={x:last.x-ux*4,y:last.y-uy*4};
    const pts=[start,...centres.slice(1,-1),shaftEnd],points=pts.map(p=>\`${'${p.x},${p.y}'}\`).join(' ');
    const outerTip={x:last.x+ux*3,y:last.y+uy*3},outerBase={x:last.x-ux*6,y:last.y-uy*6};
    const outerPoints=\`${'${outerTip.x},${outerTip.y} ${outerBase.x+px*4},${outerBase.y+py*4} ${outerBase.x-px*4},${outerBase.y-py*4}'}\`;
    const innerTip={x:last.x+ux*2,y:last.y+uy*2},innerBase={x:last.x-ux*5,y:last.y-uy*5};
    const innerPoints=\`${'${innerTip.x},${innerTip.y} ${innerBase.x+px*3},${innerBase.y+py*3} ${innerBase.x-px*3},${innerBase.y-py*3}'}\`;
